...
class SuccessfulPasswordResetTests(TestCase):
    def setUp(self):
        email = 'john@doe.com'
        RoomUser.objects.create_user(username='john', email=email, password='123abcdef')
...
class PasswordResetConfirmTests(TestCase):
    def setUp(self):
        user = RoomUser.objects.create_user(username='john', email='john@doe.com', password='123abcdef')
        # create a valid password reset token based on how django creates the token internally:
        # https://github.com/django/django/blob/main/django/contrib/auth/forms.py#L362
    ...
    def test_view_function(self):
        view = resolve(self.url.format(uidb64=self.uid, token=self.token))
